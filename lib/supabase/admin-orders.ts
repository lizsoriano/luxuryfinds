...
// ---------------------------------------------------------------------------
// Logistics (Por ordenar / En camino): the tickets.logistics_status pipeline.
// Every ticket is generated by confirming a pedido (see app/admin/pedidos/actions.ts)
// and moves WAITING_TO_ORDER/READY_TO_ORDER -> ORDERED -> IN_TRANSIT ->
// RECEIVED_LA_PAZ -> READY_FOR_DELIVERY -> DELIVERY_SCHEDULED -> DELIVERED.
// ---------------------------------------------------------------------------

export type LogisticsTicketRow = {
  id: string;
  ticket_number: string;
  order_item_id: string;
  product_name_snapshot: string;
...
export async function listTicketsByLogistics(statuses: string[]): Promise<LogisticsTicketRow[]> {
  const { data, error } = await adminDb()
    .from("tickets")
    .select(
      "id, ticket_number, order_item_id, product_name_snapshot, variant_name_snapshot, variant_id, quantity, logistics_status, created_at, clients(first_name, last_name, phone), order_items(store_name, notes)",
    )
    .in("logistics_status", statuses)
    .order("created_at", { ascending: true });
  if (error) throw new Error(error.message);

  return (data ?? []).map((row) => {
    const client = relation(row.clients as unknown as { first_name: string; last_name: string; phone: string }[]);
    const orderItem = relation(row.order_items as unknown as { store_name: string | null; notes: string | null }[]);
    return {
      id: row.id as string,
      ...
      variant_name_snapshot: row.variant_name_snapshot as string | null,
      variant_id: row.variant_id as string,
      quantity: Number(row.quantity),
      logistics_status: row.logistics_status as string,
      created_at: row.created_at as string,
      clientName: client ? `${client.first_name} ${client.last_name}`.trim() : "Clienta eliminada",
      clientPhone: client?.phone ?? "—",
      storeName: orderItem?.store_name ?? null,
